    ...
    if isinstance(obj, dict):
        ...
        for key in dictionary:
            ...
            if isinstance(dictionary[key], dict):
                ...
                json_key = key_to_json(key)
                json_string = json_dict(json_key, json_dumps(dictionary[key]), dict_counter, len(dictionary), json_string)
        ...
        for item in obj:
            ...
            if isinstance(item, (list, tuple, dict)):
                json_string = json_list(json_dumps(item), list_counter, len(obj), json_string)

            else:
                json_item = value_to_json(item)
                json_string = json_list(str(json_item), list_counter, len(obj), json_string)
...
def json_list(item, counter, length, json_string):
